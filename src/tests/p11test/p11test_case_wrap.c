...
static int check_encrypt_decrypt_secret(CK_BYTE *plain_key, CK_ULONG plain_key_len, test_cert_t *pkcs11_key, token_info_t *info)
{
...
static int test_unwrap_aes(test_cert_t *o, token_info_t *info, test_mech_t *mech)
{
	CK_FUNCTION_LIST_PTR fp = info->function_pointer;
	CK_MECHANISM mechanism = { mech->mech, NULL_PTR, 0 };
	/* SoftHSM supports only SHA1 with OAEP encryption */
	...
	debug_print(" [ KEY %s ] Unwrap a AES key using CKM_%s", o->id_str,
	            get_mechanism_name(mech->mech));
	...
	mechanism.pParameter = mech->params;
	mechanism.ulParameterLen = mech->params_len;
	rv = fp->C_UnwrapKey(info->session_handle, &mechanism, o->private_handle, wrapped, wrapped_len, template,
					     template_len, &tmp_key.private_handle);
	free(wrapped);
	if (rv != CKR_OK) {
		mech->params = NULL;
		mech->params_len = 0;
